...
class McDecayModeSummary {
  ...
  public:
    ...
    const std::unordered_map<Vertex,std::vector<Vertex>>& tau_modes() const;
    std::unordered_map<Vertex, std::vector<Vertex>>& tau_modes();
  ...
  private:
    ...
    std::unordered_map<Vertex, std::vector<Vertex>> tau_modes_;
};
